...
export default function SettingsPage() {
  ...
  return (
    <div className="mx-auto max-w-[680px] px-5 py-5">
      <h1 className="mb-5 text-[18px] font-bold tracking-tight">설정</h1>

      <section className="mb-6 rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        <h2 className="mb-1 flex items-center gap-1.5 text-[13.5px] font-bold"><Download size={14} /> 백업</h2>
        <p className="mb-3 text-[12.5px] text-zinc-400">전체 데이터(워크스페이스·프로젝트·태스크·캔버스·섹션)를 JSON 파일로 내보냅니다. 주기적으로 받아두세요.</p>
      ...
      <section className="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        <h2 className="mb-1 flex items-center gap-1.5 text-[13.5px] font-bold"><CalendarCheck2 size={14} /> 구글캘린더</h2>
        {!gcalEnabled ? (
            ...
            <div className="space-y-1">
              {gcal.calendars.map(c => (
                <label key={c.id} className="flex cursor-pointer items-center gap-2 rounded-md px-1.5 py-1 hover:bg-zinc-50 dark:hover:bg-zinc-800/60">
                  ...
                  <span className="text-[12.5px]">{c.summary}</span>
                  {c.primary && <span className="text-[10.5px] font-semibold text-zinc-400">기본</span>}
                </label>
              ))}
            </div>
          </div>
