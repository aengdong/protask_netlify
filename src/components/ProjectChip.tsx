...
export default function ProjectChip({ projectId, workspaceId }: { projectId: string | null; workspaceId: string | null }) {
  ...
  if (project) {
    ...
    return (
      <span
        className="inline-flex max-w-[150px] items-center rounded-full px-2 py-px text-[11px] font-medium text-white"
  ...
  return (
    <span
      className="inline-flex max-w-[150px] items-center gap-1 rounded-full border border-zinc-200 bg-zinc-50 px-1.5 py-px text-[11px] font-medium text-zinc-500 dark:border-zinc-700 dark:bg-zinc-800/70 dark:text-zinc-400"
      title={ws.name}
    >
